...
# ---------------------------------------------------------------------------
# Ion — Gymnasium MuJoCo training
# ---------------------------------------------------------------------------

def train_ion_mujoco(config: MuJoCoConfig, env_id: str, seed: int) -> dict:
    threshold = _ENV_META[env_id]["threshold"]
...
def run_experiment(
    ...
    if impl == "linen":
        train_fn = train_linen_mujoco
    elif impl == "nnx":
        train_fn = train_nnx_mujoco
    else:
        train_fn = train_ion_mujoco
    monitor.start()
